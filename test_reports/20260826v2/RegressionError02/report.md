...
The non-migrated DS candidate `900540 | Spiran 9, S:t Persgatan 95 | Not Migrated` is still discoverable through `Admin -> Migrate DS`, but the matching service-portal result routes into the legacy rental host instead of staying in the intended stage purchase flow.
...
6. Observe the matching result entry and continue into it.

## Actual Result

- Both the result title and the `Hyr plats` CTA expose `https://web-stage.europark.local/lgcy/garage/details/4d19a78b-9c55-451c-8a6c-1a7070fddd17`.
...
- The service-portal result remained discoverable, but the route itself was legacy-oriented before the detail page even opened.
- The session was restored to the logged-in `myaccount/index` page after the reproductions.
