...
| `regression-serviceportal-checkout-verify-and-create-contract` | G - Checkout verification and create contract | `invalidated` | Historical observation only. The checkout failure was not retried across at least ten distinct migrated DS candidates, which is now required before classifying a stage checkout failure as a confirmed regression. | `RegressionError02\report.md` |
