...
| `raw_data\ds-routing-inventory.json` | Maskinläsbar inventering av DS-nummer, DS-namn och routing från `CreateNewContract` steg 1 | Stage-ögonblicksbild; visar routing i testmiljön och ska uppdateras via regressionstest `J` | Val av SPS- eller legacy-DS i regressioner och jämförelse mot migreringsreferens | `syntetisk_data\common\ds-routing-index.md`<br>`syntetisk_data\feature\garage\garage-ds-setup-och-platser.md`<br>`syntetisk_data\feature\kontrakt\skapa-kontrakt.md` | Uppdaterad - ny gemensam DS-routingkälla skapad från Kundtjänst-GUI:ts dropdown och klassificerad via kontraktsskapandets steg 1. |
