...
| `G` | `B -> G` | `regression-serviceportal-checkout-verify-and-create-contract` | Start from the checkout page created by B, verify checkout and contract creation, and retry the full B -> G flow across at least ten migrated DS candidates before treating a stage failure as a confirmed regression. | `testing\regression_test\serviceportal-checkout-verifiering-och-skapa-kontrakt.md` |
...
- `G` bygger vidare på slutläget i `B` och verifierar checkout- samt skapa-kontrakt-steget. Om `G` fallerar ska hela `B -> G`-kedjan upprepas med nya migrerade DS-kandidater tills ett DS passerar eller minst 10 olika migrerade DS visar samma systemiska felkategori.
